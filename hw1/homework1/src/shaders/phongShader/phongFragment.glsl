...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  #endif
  int num_of_blocks = 0;
  float sum_of_blocks = 0.0;
  float shadowStride = 20.0;
  float filterRange = 1.0 / shadowStride * 400.0;

  for(int i = 0; i < NUM_SAMPLES; ++i){
    float depth = unpack(texture2D(shadowMap, uv + poissonDisk[i] * filterRange/ RESOLUTION)) + 0.005;
    if(depth < zReceiver){
      ++num_of_blocks;
      sum_of_blocks += depth;
    }
  }
  if(num_of_blocks == NUM_SAMPLES){
    return 2.0;
  }
  if(num_of_blocks == 0)
    return 0.0;
  return sum_of_blocks / float(num_of_blocks);
}
...
float PCF(sampler2D shadowMap, vec4 coords, float filterRadius) {
  ...
  float result = 0.0;
  for(int i = 0; i < NUM_SAMPLES; ++i){
    float depth = unpack(texture2D(shadowMap, coords.xy + poissonDisk[i] * filterRadius / RESOLUTION)) + 0.005;
    result += depth > coords.z ? 1.0 : 0.6;
  }
...
float PCSS(sampler2D shadowMap, vec4 coords){
  ...
  float penumbra = ((coords.z - avgDepth) *0.9 / avgDepth);  
  ...
  //return shadow < 0.01 ? 0.4 : 1.0;
  return (shadow * 1.5) < 1.0 ? (shadow * 1.5) : 1.0;
}

float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  vec3 lightDir = normalize(uLightPos);
  vec3 normal = normalize(vNormal);
  float depth = unpack(texture2D(shadowMap, shadowCoord.xy)) + 0.005;
  return depth > shadowCoord.z ? 1.0 : 0.6;
}
